...
const RegionsProvider = ({ children }) => {
  ...
  // geneset enrichment states
  const [activeGenesetEnrichment, setActiveGenesetEnrichment] = useState(null)
  const [genesetEnrichmentLoading, setGenesetEnrichmentLoading] = useState(false)

  // region set factor enrichment states
  ...
  // Number of top regions to display in the UI
  ...
  useEffect(() => {
    if(filteredActiveRegions?.length) {
      ...
      setGenesetEnrichmentLoading(true)
      ...
      fetchCombinedPathsAndGWAS(narrationRegions).then((response) => {
        ...
        setGenesetEnrichmentLoading(false)
      })
      .catch((e) => {
  ...
  // generates region set narration
  ...
  }, [topNarrations, activeGenesetEnrichment, setRegionSetNarrationLoading])
  ...
  }, [regionSetQuery, setRegionSetNarrationLoading])
